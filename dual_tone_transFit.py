...
try:
	subdf = df.loc['20191008T122246']
except KeyError:
	print ('Cant locate shot 20191008T122246 in lyse.')
...
ac_amp = globs['magnetom_rabi_spectrum_amps'][1]
ac_freq = globs['magnetom_rabi_spectrum_freqs'][1]
fsweep_requested = (globs['magnetom_rabi_sweep_initial'], globs['magnetom_rabi_sweep_final'])
...
if ('rabi_sweep_fitting','amp_analytic') in dataset:
	...
	result_string_an2 = r'$\Omega_{{2S}}$ = {:.0f}({:.0f}) Hz'.format(f_rabi2_a, u_f_rabi2_a)
...
fig = plt.figure('dual_tone_transFit', figsize = (10,6))

# Plot and label spectrogram
ax1 = fig.add_subplot(2,1,1)
# ax1.plot_stft(stft, t, f, range=spectrogram_lims, png=None)
ax1.set_title('Dual-tone signal: second transition')
...
ax2.text(textCoords_x(0.042), textCoords_y(0.587+0.25), 'fit results:', c = colours[0])
ax2.text(textCoords_x(0.042), textCoords_y(0.411+0.25), result_string_ad, c = colours[1])
ax2.text(textCoords_x(0.042), textCoords_y(0.245+0.25), result_string_an, c = 'k')
ax2.text(textCoords_x(0.042), textCoords_y(0.088+0.25), result_string_exp, c = 'g')
# textObjs = [t1,t2,t3]
...
ax2.add_patch(patches.Rectangle((textCoords_x(0.03), textCoords_y(0.0588+0.25)), 0.205*trange, 0.62*Qrange, facecolor = 'lightgrey', edgecolor = 'k', alpha = 0.5))

plt.show()

# Save figures
plt.savefig(os.path.join(folder, 'dual_tone_transFit.png'), dpi = 300, bbox_inches='tight')
plt.savefig(os.path.join(folder, 'dual_tone_transFit.pdf'), dpi = 300, bbox_inches='tight')
