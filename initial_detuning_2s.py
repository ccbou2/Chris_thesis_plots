...
if ('rabi_sweep_fitting','amp') in dataset:
	...
	result_string_ad2 = r'$\Omega_{{2S}}$ = {:.2f}({:.0f}) kHz'.format(f_rabi2*1e-3, u_f_rabi2*1e-1)
...
fig = plt.figure('init_detuning_2s', figsize = (10,6))

# Plot and label spectrogram
ax1 = fig.add_subplot(2,1,1)
# ax1.plot_stft(stft, t, f, range=spectrogram_lims, png=None)
ax1.set_title('Effect of initial detuning, $\Delta_i = 2$ kHz')
...
# Save figures
plt.savefig(os.path.join(folder, 'init_detuning_2s.png'), dpi = 300, bbox_inches='tight')
plt.savefig(os.path.join(folder, 'init_detuning_2s.pdf'), dpi = 300, bbox_inches='tight')
