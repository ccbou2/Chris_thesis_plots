...
# 	result_string_ad1 = r'$f_2$ = {:.3f}({:.0f}) kHz'.format(f_res/1e3, u_f_res)
# 	result_string_ad2 = r'$\Omega_2$ = {:.0f}({:.0f}) Hz'.format(f_rabi2, u_f_rabi2)
# 	result_string_ad = result_string_ad1 + '\n' + result_string_ad2
# 	result_string_exp1 = r'exp $f_2$ = {:.0f} kHz'.format(f_res_exp/1e3)
# 	result_string_exp2 = r'exp $\Omega_2$ = {:.0f} Hz'.format(f_rabi2_exp)
...
# 	result_string_an1 = r'$f_2$ = {:.3f}({:.0f}) kHz'.format(f_res_a/1e3, u_f_res_a)
# 	result_string_an2 = r'$\Omega_2$ = {:.1f}({:.0f}) Hz'.format(f_rabi2_a, u_f_rabi2_a*10)
...
fig = plt.figure('concat_decoupling', figsize = (10,6))
...
# Save figures
plt.savefig(os.path.join(folder, 'concat_decoupling.png'), dpi = 300, bbox_inches='tight')
plt.savefig(os.path.join(folder, 'concat_decoupling.pdf'), dpi = 300, bbox_inches='tight')
