...
if ('rabi_sweep_fitting','amp_analytic') in dataset:
	...
	result_string_an1 = r'$f_{{2S}}$ = {:.3f}({:.0f}) kHz'.format(f_res_a/1e3, u_f_res_a)
...
fig = plt.figure('single_tone_LZ', figsize = (10,6))

# Plot and label spectrogram
ax1 = fig.add_subplot(2,1,1)
# ax1.plot_stft(stft, t, f, range=spectrogram_lims, png=None)
ax1.set_title('Landau-Zener transition driven by a single-tone signal')
...
# Save figures
plt.savefig(os.path.join(folder, 'single_tone_LZ.png'), dpi = 300, bbox_inches='tight')
plt.savefig(os.path.join(folder, 'single_tone_LZ.pdf'), dpi = 300, bbox_inches='tight')
print('Plotting complete, outputs saved to ' + str(folder))

spec_ylims2 = np.array((larmor - np.max(rabi_freqs_sub), larmor + np.max(rabi_freqs_sub)))/1e3
# spec_ylims = np.array((larmor - np.max(rabi_freqs_lockin) - 4e3, larmor + np.max(rabi_freqs_lockin) + 4e3))/1e3
demod_ylims2 = (-max(filter_Q_sub)*1e3-0.05, max(filter_Q_sub)*1e3+0.05)
# demod_ylims = (-max(filter_Q)*1e3-0.05, max(filter_Q)*1e3+0.05)
time_lims2 = (np.min(t_lockin_sub), np.max(t_lockin_sub))
# time_lims = (np.min(t_lockin), np.max(t_lockin))
trange2 = time_lims2[1] - time_lims2[0]
Qrange2 = demod_ylims2[1] - demod_ylims2[0]
# spectrogram_lims = [1, 3.5]

# Define figure object
fig3 = plt.figure('single_tone_LZ_2', figsize = (10,6))

# Plot and label spectrogram
ax31 = fig3.add_subplot(2,1,1)
# ax1.plot_stft(stft, t, f, range=spectrogram_lims, png=None)
ax31.set_title('Landau-Zener transition driven by a single-tone signal')
ax31.imshow(np.log10(stft.T), origin='lower', vmin=spectrogram_lims[0], vmax=spectrogram_lims[1], extent=[t[0], t[-1], f[0]/1e3, f[-1]/1e3], aspect='auto', interpolation='nearest', cmap='jet')
ax31.axvline(ac_res_time_lockin, c = 'm', ls = '--')
# ax31.axvline(bound_times_lockin[0], c = 'orange', ls = '--')
# ax31.axvline(bound_times_lockin[1], c = 'orange', ls = '--')
ax31.set_xlabel('Time [s]')
ax31.set_ylabel('Frequency [kHz]')
ax31.set_xlim(time_lims2)
ax31.set_ylim(spec_ylims2)

# Make second subplot for demod channel, w/ fits
ax32 = fig3.add_subplot(2,1,2)
ax32.spines["top"].set_visible(False)
ax32.axhline(0, c = 'lightgray', alpha = 0.5)
ax32.plot(t_lockin, filter_Q*1e3, label = 'filtered Q(t)')
ax32.plot(t_lockin_sub, adiabaticFitRecon*1e3, ls = '--', label = 'adiabatic fit')
# ax2.plot(t_lockin_sub, analyticFitRecon*1e3, ls = '--', c = 'k', label = 'analytic fit')
ax32.axvline(ac_res_time_lockin, c = 'm', ls = '--', label = r'$f_{{2A}}$ = {:.2f} kHz'.format(ac_res_rabi_freq/1e3))
# ax32.axvline(bound_times_lockin[0], c = 'orange', ls = '--', label = 'Sweep bounds')
ax32.axvline(bound_times_lockin[1], c = 'orange', ls = '--')
ax32.set_xlabel('Time [s]')
ax32.set_ylabel('Q(t) [mV]')
ax32.set_ylim(demod_ylims2)
ax32.set_xlim(time_lims2)
ax32.legend()
plt.subplots_adjust(hspace = 0)

# Define ticks for Rabi frequency
rabi_tick_locs = ax32.get_xticks()
rabi_tick_freqs = np.around(poly_freq_calib(amp_map(rabi_tick_locs, t0, rabiT, amp_bounds[0], amp_bounds[1]))/1e3, 2)

# Make secondary axis for Rabi frequency on spectrogram
ax312 = ax31.twiny()
ax312.set_xticks(rabi_tick_locs)
ax312.set_xticklabels(rabi_tick_freqs)
ax312.set_xlim(ax32.get_xlim())
ax312.set_xlabel(r'$\Omega_{{1R}}$ [kHz]')
ax31.xaxis.set_visible(False)
ax31.spines["bottom"].set_visible(False)

# Convert text coords into positions based on plot ranges
def textCoords_x(coord, tmin=time_lims2[0], trange=trange2):
	return tmin + coord*trange
def textCoords_y(coord, Qmin=demod_ylims2[0], Qrange=Qrange2):
	return Qmin + coord*Qrange
...
# ax2.text(0.21, -0.4, result_string_an, c = 'k')
# ax2.text(0.21, -0.7, result_string_exp, c = 'g')
# ax32.text(textCoords_x(0.042), textCoords_y(0.587), 'fit results:', c = colours[0])
ax32.text(textCoords_x(0.042), textCoords_y(0.411), 'fit results:', c = colours[0])
# ax32.text(textCoords_x(0.042), textCoords_y(0.411), result_string_ad, c = colours[1])
ax32.text(textCoords_x(0.042), textCoords_y(0.245), result_string_ad, c = colours[1])
# ax2.text(textCoords_x(0.042), textCoords_y(0.245), result_string_an, c = 'k')
ax32.text(textCoords_x(0.042), textCoords_y(0.088), result_string_exp, c = 'g')
# textObjs = [t1,t2,t3]
...
# ax2.add_patch(patches.Rectangle((0.208, -0.50), 0.023, 0.9, facecolor = 'white', edgecolor = 'k', alpha = 0.5))
# ax2.add_patch(patches.Rectangle((0.208, -0.75), 0.023, 1.15, facecolor = 'white', edgecolor = 'k', alpha = 0.5))
ax32.add_patch(patches.Rectangle((textCoords_x(0.03), textCoords_y(0.0588)), 0.192*trange2, 0.45*Qrange2, facecolor = 'lightgrey', edgecolor = 'k', alpha = 0.5))

# Save figures
plt.savefig(os.path.join(folder, 'single_tone_LZ_adiabatic.png'), dpi = 300, bbox_inches='tight')
plt.savefig(os.path.join(folder, 'single_tone_LZ_adiabatic.pdf'), dpi = 300, bbox_inches='tight')
print('Plotting complete, outputs saved to ' + str(folder))
