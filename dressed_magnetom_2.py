...
maxDemodAmp = np.max((np.abs(np.max(filter_Q)), np.abs(np.min(filter_Q))))
# demod_ylims = (-maxDemodAmp*1e3-0.05, maxDemodAmp*1e3+0.05)
demod_ylims = (-maxDemodAmp*1e3-0.05, maxDemodAmp*1e3-0.2)
# time_lims = (np.min(t_lockin_sub), np.max(t_lockin_sub))
# time_lims_1 = (np.min(t_lockin), np.max(t_lockin))
time_lims_1 = (np.min(t_lockin), 0.4)
# time_lims_zoom = (tsweep[0],tsweep[1]-0.05)
time_lims_zoom = tsweep
trange1 = time_lims_1[1] - time_lims_1[0]
trange_zoom = time_lims_zoom[1] - time_lims_zoom[0]
Qrange = demod_ylims[1] - demod_ylims[0]
demod_lims_zoom = (demod_ylims[0], 0.001)
...
axTitle.set_title(r'Dressed AC magnetometry - 200$\mu$V signal', y = 1.04)
axTitle.axis('off')
...
# Save figures
plt.savefig(os.path.join(folder, 'dressed_magnetom_2.png'), dpi = 300, bbox_inches='tight')
plt.savefig(os.path.join(folder, 'dressed_magnetom_2.pdf'), dpi = 300, bbox_inches='tight')
